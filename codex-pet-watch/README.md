...
The idea is intentionally simple: watch a small 40x32 bottom-left rectangle where the Codex pet sits, offset 32 px from the left edge and 64 px from the bottom edge, compare captured pixels, and play a sound when the pet changes visually.
...
The Windows version is the current working workaround. It launches without args, watches a 40x32 DIP rectangle offset 32 px from the left edge and 64 px from the bottom edge, plays `C:\Windows\Media\ringout.wav`, and exits from the tray menu.
