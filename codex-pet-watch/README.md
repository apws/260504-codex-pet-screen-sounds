...
Tiny Win32/GDI watcher for a `codex-pet-area` rectangle anchored at the bottom-left of the active monitor.
...
Relative sound paths are resolved against the folder containing `codex_pet_watch.exe`. For example, if you run:

```bat
codex_pet_watch.exe alert.wav
```

then `alert.wav` should sit next to the exe.
...
```bat
codex_pet_watch.exe alert.wav
codex_pet_watch.exe alert.wav 360 240
codex_pet_watch.exe alert.wav --size=480x280 --poll-ms=1000 --work-area
codex_pet_watch.exe alert.wav --size=360x240 --follow-foreground
```

Options:

```text
--size=WxH              Rectangle size in logical DIP units. Default: 360x240
...
Internally, the overlay position and GDI capture rectangle use physical desktop pixels. This matters for multi-monitor setups, DPI scaling, and monitors with negative coordinates.
...
- The app is a console app so you can stop it with `Ctrl+C` from the console.
