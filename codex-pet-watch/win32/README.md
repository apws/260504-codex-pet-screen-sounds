...
```bat
codex_pet_watch.exe alert.wav
codex_pet_watch.exe --size=80x80
codex_pet_watch.exe alert.wav 80 80
codex_pet_watch.exe alert.wav --size=480x280 --poll-ms=1000 --work-area
codex_pet_watch.exe alert.wav --size=80x80 --follow-foreground
```

Options:

```text
--size=WxH              Rectangle size in logical DIP units. Default: 80x80
