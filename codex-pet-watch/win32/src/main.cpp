...
constexpr int kDefaultWidthDip = 24;
constexpr int kDefaultHeightDip = 24;
constexpr int kLeftOffsetPx = 56;
constexpr int kBottomOffsetPx = 72;
...
void PrintUsage() {
    std::fwprintf(stderr,
        ...
        L"Options:\n"
        L"  --size=WxH              Rectangle size in logical DIP units. Default: 24x24\n"
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == L"--help" || a == L"-h" || a == L"/?") {
            ...
            if (!ParseSizeValue(a.substr(7), cfg.widthDip, cfg.heightDip)) {
                std::fwprintf(stderr, L"Invalid --size value. Use --size=24x24.\n");
                return false;
            }
