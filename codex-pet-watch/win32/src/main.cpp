...
constexpr int kLeftOffsetPx = 64;
constexpr int kBottomOffsetPx = 80;
...
void PrintUsage() {
    std::fwprintf(stderr,
        ...
        L"Options:\n"
        L"  --size=WxH              Rectangle size in logical DIP units. Default: 16x16\n"
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == L"--help" || a == L"-h" || a == L"/?") {
            ...
            if (!ParseSizeValue(a.substr(7), cfg.widthDip, cfg.heightDip)) {
                std::fwprintf(stderr, L"Invalid --size value. Use --size=16x16.\n");
                return false;
            }
