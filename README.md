...
Small native workaround tool that watches the Codex desktop pet pixels and plays a sound when the pet changes visually. It is intentionally simple: capture a tiny 48x48 bottom-left rectangle offset 32 px from the left edge and 48 px from the bottom edge, compare pixels once per second, and ring when anything changes.
...
The Windows app launches without args, lives in the system tray, watches a 48x48 DIP rectangle near the bottom-left of the screen, offset 32 px from the left edge and 48 px from the bottom edge, and plays `C:\Windows\Media\ringout.wav` when the watched pixels change. Right-click the tray icon and choose `Exit` to stop it.
