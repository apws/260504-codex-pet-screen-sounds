...
The Windows app launches without args, lives in the system tray, watches a 16x16 DIP rectangle in the upper-right area of the bottom-left parked pet sprite, offset 64 px from the screen's left edge and 80 px from the screen's bottom edge, and plays the embedded default `ringout.wav` when the watched pixels change. Right-click the tray icon and choose `Exit` to stop it.
